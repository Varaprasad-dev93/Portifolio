...
Experience the wonders of space with this visually engaging and educational 3D model! 🚀`
  ...
  const ProjectInfo=(props)=>{
    ...
    return(
        <Mdiv className='flex overflow-y-scroll custom-scrollbar gap-10 backdrop-blur-lg bg-white/10 border border-white/20 rounded-2xl shadow-lg hover:border-white transition-transform duration-300' key={val}
        initial={
          {
            opacity:0,
          }
        ...
        <div  className='z-1'>
          ...
          <Button className='px-6 py-2 border border-white/30 bg-white/10 backdrop-blur-lg text-white rounded-lg shadow-md transition-all hover:bg-white/20 hover:border-white/50'
          onClick={() => window.open(project.githublink, "_blank")}>Github</Button>
          <Button className='px-6 py-2 border border-white/30 bg-white/10 backdrop-blur-lg text-white rounded-lg shadow-md transition-all hover:bg-white/20 hover:border-white/50'
          onClick={() => window.open(project.link, "_blank")}>Live Demo</Button>
          </div>
        </div>
        </Mdiv>
    )
